...
/**
 * Created by chenshu on 02/03/2017.
 */var config$6={props:{setups:{type:Array,default:function _default(){return[];}},views:{type:Array,default:function _default(){return[];}},route:{type:Object,default:function _default(){return{};}}},mounted:function mounted(){this.createNavi();},methods:{addViewHotspot:function addViewHotspot(it,active){var spotname='view_'+(it+1);var url='%FIRSTXML%/interface/'+(it+1)+'_'+active+'.png';var h=-22+it*6;var v=40;this.krpanoObj.call('\n        addhotspot('+spotname+');\n        set(hotspot['+spotname+'].url, '+url+');\n        set(hotspot['+spotname+'].ath, '+h+');\n        set(hotspot['+spotname+'].atv, '+v+');\n        set(hotspot['+spotname+'].scale, .25);\n        set(hotspot['+spotname+'].distorted, true);\n        set(hotspot['+spotname+'].onclick, jscall(calc(\'krpano.hooks.gotoView('+it+')\')) );\n      ');},addSetupHotspot:function addSetupHotspot(it,active){var spotname='setup_'+(it+1);var url='%FIRSTXML%/interface/setup_'+(it+1)+'_'+active+'.png';var h=-18+this.views.length*6+it*6;var v=40;this.krpanoObj.call('\n        addhotspot('+spotname+');\n        set(hotspot['+spotname+'].url, '+url+');\n        set(hotspot['+spotname+'].ath, '+h+');\n        set(hotspot['+spotname+'].atv, '+v+');\n        set(hotspot['+spotname+'].scale, .25);\n        set(hotspot['+spotname+'].distorted, true);\n        set(hotspot['+spotname+'].onclick, jscall(calc(\'krpano.hooks.gotoSetup('+it+')\')) );\n      ');},createNavi:function createNavi(){for(var i=0;i<this.views.length;i++){var routeView=Number(this.route.params.view)||0;var active=routeView===i?'off':'on';this.addViewHotspot(i,active);}for(var _i=0;_i<this.setups.length;_i++){var routeSetup=Number(this.route.params.setup)||0;var _active=routeSetup===_i?'off':'on';this.addSetupHotspot(_i,_active);}}},watch:{route:function route(){this.createNavi();}}};
